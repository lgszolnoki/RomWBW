...
#ENDIF
#IF (FDMODE == FDMODE_RCSMC4)
FDC_MSR		.EQU	$B0		; FDC9266 MAIN STATUS REGISTER
FDC_DATA	.EQU	$B1		; FDC9266 DATA PORT
FDC_DOR		.EQU	$B8		; DIGITAL OUTPUT REGISTER (LATCH)
  #DEFINE FDMODE_STR "RCSMC4DRV"
#ENDIF
...
FD_CFGTBL:
					; DEVICE 0
	.DB	FD0DRV			; DEVICE NUMBER
	.DB	0			; DEVICE STATUS
	.DB	FD0MED			; $FF	; CURRENT MEDIA TYPE (INIT TO NONE)
	.DB	$FF			; CURRENT TRACK
	.DW	0			; HOST TRACK
	.DB	0			; HOST SECTOR
	.DB	0			; HOST HEAD
	.DB	FD0TYPE			; DRIVE TYPE
FD_CFGSIZ	.EQU ($ - FD_CFGTBL)
;
	DEVECHO	"FD: MODE="
	DEVECHO	FDMODE_STR
	DEVECHO	", IO="
...
#IF (FDCNT >= 2)
					; DEVICE 1
	.DB	FD1DRV			; DRIVER DEVICE NUMBER
	.DB	0			; DEVICE STATUS
	.DB	FD1MED			;$FF	; CURRENT MEDIA TYPE (INIT TO NONE)
	.DB	$FF			; CURRENT TRACK
	.DW	0			; HOST TRACK
	.DB	0			; HOST SECTOR
	.DB	0			; HOST HEAD
...
#IF (FDCNT >= 3)
					; DEVICE 2
	.DB	FD2DRV			; DRIVER DEVICE NUMBER
	.DB	0			; DEVICE STATUS
	.DB	FD2MED			;$FF	; CURRENT MEDIA TYPE (INIT TO NONE)
	.DB	$FF			; CURRENT TRACK
	.DW	0			; HOST TRACK
	.DB	0			; HOST SECTOR
	.DB	0			; HOST HEAD
	.DB	FD2TYPE			; DRIVE TYPE
;
	DEVECHO	"FD: MODE="
	DEVECHO	FDMODE_STR
	DEVECHO	", IO="
...
#IF (FDCNT >= 4)
					; DEVICE 3
	.DB	FD3DRV			; DRIVER DEVICE NUMBER
	.DB	0			; DEVICE STATUS
	.DB	FD3MED			;$FF	; CURRENT MEDIA TYPE (INIT TO NONE)
	.DB	$FF			; CURRENT TRACK
	.DW	0			; HOST TRACK
	.DB	0			; HOST SECTOR
	.DB	0			; HOST HEAD
	.DB	FD3TYPE			; DRIVE TYPE
;
	DEVECHO	"FD: MODE="
	DEVECHO	FDMODE_STR
	DEVECHO	", IO="
...
FDMM_5DD	.DB	FDM360, FDMRO624, FDMRO800, FDMRO780, FDMRO720, FDMRO520, FDMRO506, $FF
...
FCD_RO624
		...
		.DB	16				; NUMBER OF SECTORS
		.DB	1				; START OF TRACK (ID OF FIRST SECTOR, USUALLY 1)
		.DB	16				; SECTOR COUNT
		.DW	256				; SECTOR SIZE IN BYTES
		.DB	36H				; 0AH				; GAP LENGTH (R/W)
		.DB	0EH				; 0CH				; GAP LENGTH (FORMAT)
		.DB	(14 << 4) | 0	; SRT = 4ms, HUT = 512ms
		.DB	(8 << 1) | 1	; HLT = 50ms, ND = YES
		.DB	DOR_BR250		; DOR
		.DB	DCR_BR250		; DCR
		.IF	(($ - FCD_RO624) != FCD_LEN)
		.ECHO	"*** FCD_RO624 SIZE ERROR!!! ***\n"
		.ENDIF
;
;*************************** robotron **********************
FCD_RO800
		...
		.DB	35H             ;     080H			; GAP LENGTH (R/W)
		.DB	74H             ;0F0H			; GAP LENGTH (FORMAT)
		.DB	(14 << 4) | 0	; SRT = 4ms, HUT = 512ms
		.DB	(8 << 1) | 1	; HLT = 50ms, ND = YES
		.DB	DOR_BR250		; DOR
		.DB	DCR_BR250		; DCR
...
FCD_RO780
		...
		.DB	DOR_BR250		; DOR
		.DB	DCR_BR250		; DCR
		.IF	(($ - FCD_RO780) != FCD_LEN)
		.ECHO	"*** FCD_RO780 SIZE ERROR!!! ***\n"
		.ENDIF
;
;*************************** robotron **********************
FCD_RO720
		...
		.DB	54H				; GAP LENGTH (R/W)
		.DB	1BH				; GAP LENGTH (FORMAT)
		.DB	(14 << 4) | 0	; SRT = 4ms, HUT = 512ms
		.DB	(8 << 1) | 1	; HLT = 50ms, ND = YES
		.DB	DOR_BR250		; DOR
		.DB	DCR_BR250		; DCR
		.IF	(($ - FCD_RO720) != FCD_LEN)
		.ECHO	"*** FCD_RO720 SIZE ERROR!!! ***\n"
		.ENDIF
;
;*************************** robotron **********************
FCD_RO520
		.DB	80				; NUMBER OF CYLINDERS
		.DB	2				; NUMBER OF HEADS
		.DB	26				; NUMBER OF SECTORS
		.DB	1				; START OF TRACK (ID OF FIRST SECTOR, USUALLY 1)
		.DB	26				; SECTOR COUNT
		.DW	128				; SECTOR SIZE IN BYTES
		.DB	$10				; GAP LENGTH (R/W)
		.DB	$19				; GAP LENGTH (FORMAT)
		.DB	(14 << 4) | 0	; SRT = 4ms, HUT = 512ms
		.DB	(8 << 1) | 1	; HLT = 50ms, ND = YES
		.DB	DOR_BR250		; DOR
		.DB	DCR_BR250		; DCR
		.IF	(($ - FCD_RO520) != FCD_LEN)
		.ECHO	"*** FCD_RO520 SIZE ERROR!!! ***\n"
		.ENDIF
;
;*************************** robotron **********************
FCD_RO506
		...
		.DB	DOR_BR250		; DOR
		.DB	DCR_BR250		; DCR
		.IF	(($ - FCD_RO506) != FCD_LEN)
		.ECHO	"*** FCD_RO506 SIZE ERROR!!! ***\n"
		.ENDIF
...
;**************************** robotron **********************
		LD	HL,FCD_RO624 \ RET	; FDMRO624 = 5
		LD	HL,FCD_RO800 \ RET	; FDMRO800 = 6
		LD	HL,FCD_RO780 \ RET	; FDMRO780 = 7
		LD	HL,FCD_RO720 \ RET	; FDMRO720 = 8
		LD	HL,FCD_RO520 \ RET	; FDMRO520 = 9
		LD	HL,FCD_RO506 \ RET	; FDMRO506 = 10
...
;
#IF (FDMODE == FDMODE_RCSMC4)
DOR_BR250	.EQU	10100100B	; 250KBPS W/ MOTOR OFF!
DOR_BR500	.EQU	11100000B	; 500KBPS W/ MOTOR OFF!
DOR_INIT	.EQU	11100000B	; INITIAL DEFAULT LATCH VALUE
#ENDIF
;
...
;B: 0X19, C: DISK UNIT, E: MEDIA ID, RETURNED VALUES   A: STATUS

;	LD A,D              ; CHECK FOR DEVICE TYPE
;	CP $10              ; FLOPPY,
;	JR NZ,DEFMEDERR     ; IF NOT, EXIT
	LD A,E              ;
	LD C,FD_MED         ; CALCULATE RAW MEDIA ID := INPUT - 1. FLOPPY MEDIA ID
	SUB C
	PUSH AF     ; SAVE INITIAL MEDIA ID
; test media ID, lookup in media map
...
;
	...
	RET
DEFMEDERR:
	OR -4			; NO FLOPPY, INVALID UNIT NUMBER
	RET
;
MEDLOOP:			; C NEW MEDIA ID
	LD A,(HL)		; MEDIA ID IN FD_MEDIAMAP
	CP $FF
	JR Z,MEDLOOP1		; END OF MEDIA TYPES TO TRY
	CP C
	LD A,C
	RET Z
	INC HL
...
CALC_N:
	LD	A,H
	AND	3FH             ; LIMIT SECTOR SIZE TO 8K
	RET	Z               ; SECTOR SIZE 128
	PUSH	BC
	LD	B,0
CALC_N0:
	INC	B
...
;
#IFDEF ROBOTRON
; CALCULATE FCD FROM SECTOR SIZE
	PUSH	HL
;	LD	A,$FF
;   	LD (FCD_DTL),A
	LD	HL,(FCD_SECSZ)
...
#IF ((FDMODE == FDMODE_ZETA) | (FDMODE == FDMODE_DIO3) | (FDMODE == FDMODE_RCSMC) | (FDMODE == FDMODE_RCSMC4))
	RES	7,A
#ENDIF
#IF ((FDMODE == FDMODE_DIDE) | (FDMODE == FDMODE_N8) | (FDMODE == FDMODE_ZETA2) | (FDMODE == FDMODE_RCWDC) | (FDMODE == FDMODE_DYNO) | (FDMODE == FDMODE_EPFDC) | (FDMODE == FDMODE_MBC) | (FDMODE == FDMODE_DUO))
	LD	A,0
...
FC_MOTORON1:
	...
	AND	%00000010		; ISOLATE PREVIOUS MOTOR BITS
...
FCD_N		.DB	2		; SECTOR SIZE, N=2 FOR 512 BYTES
